<x-app-layout>
    <x-slot name="header">
        <div>
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Lengkapi informasi untuk mendaftarkan alat camping/gunung baru ke dalam sistem.</p>
        </div>
    </x-slot>
                ...
                <form action="{{ route('products.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-5">
                            ...
                            <div class="space-y-1.5">
                                <label for="code" class="block text-sm font-bold text-gray-700 dark:text-gray-200">
                                    Kode Barang <span class="text-xs text-gray-400 font-normal">(Opsional - Generate Otomatis jika kosong)</span>
                                </label>
                                <input type="text" name="code" id="code" value="{{ old('code') }}" placeholder="Contoh: TSEL-TND-009" class="w-full rounded-xl border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-950 text-gray-900 dark:text-white focus:border-telkomsel-500 focus:ring-telkomsel-500/20 text-sm py-2.5">
                                @error('code')
                            ...
                            <div class="space-y-1.5">
                                ...
                                <input type="text" name="name" id="name" value="{{ old('name') }}" placeholder="Masukkan nama alat gunung (contoh: Sepatu Hiking)" class="w-full rounded-xl border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-950 text-gray-900 dark:text-white focus:border-telkomsel-500 focus:ring-telkomsel-500/20 text-sm py-2.5" required>
                                @error('name')
                                    <p class="text-xs font-semibold text-rose-500 mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div class="space-y-1.5">
                                    <label for="stock" class="block text-sm font-bold text-gray-700 dark:text-gray-200">
                                        Stok Awal <span class="text-telkomsel-500">*</span>
                                    </label>
                                    <input type="number" name="stock" id="stock" value="{{ old('stock', 0) }}" min="0" class="w-full rounded-xl border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-950 text-gray-900 dark:text-white focus:border-telkomsel-500 focus:ring-telkomsel-500/20 text-sm py-2.5" required>
                                    @error('stock')
                                        <p class="text-xs font-semibold text-rose-500 mt-1">{{ $message }}</p>
                                    @enderror
                                </div>
                                <div class="space-y-1.5">
                                    ...
                                    <input type="text" name="storage_location" id="storage_location" value="{{ old('storage_location') }}" placeholder="Contoh: Rak A-1" class="w-full rounded-xl border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-950 text-gray-900 dark:text-white focus:border-telkomsel-500 focus:ring-telkomsel-500/20 text-sm py-2.5" required>
                        ...
                        <div class="space-y-5">
                            ...
                            <div class="space-y-2">
                                <label class="block text-sm font-bold text-gray-700 dark:text-gray-200">Gambar Barang</label>
                                <div class="flex flex-col items-center p-4 border-2 border-dashed border-gray-200 dark:border-slate-800 rounded-2xl bg-gray-50 dark:bg-slate-950 text-center">
                                    ...
                                    <div id="preview-container" class="hidden w-32 h-32 mb-3 rounded-xl overflow-hidden bg-gray-150 border border-gray-200">
                                        <img id="image-preview" src="#" alt="Pratinjau Gambar" class="w-full h-full object-cover">
                                    </div>
                                    
                                    <svg id="upload-icon" class="w-10 h-10 text-gray-300 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                                    ...
                                    <p class="text-xxs text-gray-400 mt-1">PNG, JPG, JPEG, WEBP hingga 2MB</p>
                                </div>
                                @error('image')
                                    <p class="text-xs font-semibold text-rose-500 mt-1">{{ $message }}</p>
                                @enderror
                    ...
                    <div class="space-y-1.5">
                        <label for="description" class="block text-sm font-bold text-gray-700 dark:text-gray-200 font-semibold">Deskripsi / Keterangan Barang</label>
                        <textarea name="description" id="description" rows="3" placeholder="Tuliskan catatan tambahan mengenai barang gunung ini (contoh: Merk, kapasitas inner tenda, bahan pasak)..." class="w-full rounded-xl border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-950 text-gray-900 dark:text-white focus:border-telkomsel-500 focus:ring-telkomsel-500/20 text-sm py-2.5">{{ old('description') }}</textarea>
                        @error('description')
                            <p class="text-xs font-semibold text-rose-500 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
