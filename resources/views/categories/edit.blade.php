<x-app-layout>
    <x-slot name="header">
        <div>
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Ubah nama atau deskripsi kategori inventaris sesuai kebutuhan.</p>
        </div>
    </x-slot>
            ...
            <div class="bg-white dark:bg-slate-900 rounded-2xl border border-gray-150 dark:border-slate-800 shadow-sm overflow-hidden">
                <form action="{{ route('categories.update', $category->id) }}" method="POST" class="p-6 space-y-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" name="name" id="name" value="{{ old('name', $category->name) }}" placeholder="Masukkan nama kategori" class="w-full rounded-xl border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-950 text-gray-900 dark:text-white focus:border-telkomsel-500 focus:ring-telkomsel-500/20 text-sm py-2.5" required>
                        @error('name')
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea name="description" id="description" rows="4" placeholder="Tulis deskripsi..." class="w-full rounded-xl border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-950 text-gray-900 dark:text-white focus:border-telkomsel-500 focus:ring-telkomsel-500/20 text-sm py-2.5">{{ old('description', $category->description) }}</textarea>
                        @error('description')
                            <p class="text-xs font-semibold text-rose-500 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
